  ...
  def __init__(self,
    ...
    for zone in zone_names:
      vpc_subnet = ec2.Subnet(f"{subnet_name_base}-{zone}",
        ...
        cidr_block=f"{vpc_cidr.replace(vpc_cidr.split('.', 2)[2], '')}{len(self.subnets)}.0/24",
    ...
    self.rds_security_group = ec2.SecurityGroup(rds_sg_name,
      ...
      ingress=[
        ec2.SecurityGroupIngressArgs(
          cidr_blocks=[
            vpc_cidr],
          from_port=ext_args.rds_db_port,
          to_port=ext_args.rds_db_port,
          protocol="tcp",
    ...
    front_sg_name = f"{name}-front-sg"
    self.fe_security_group = ec2.SecurityGroup(front_sg_name,
      vpc_id=self.vpc.id,
