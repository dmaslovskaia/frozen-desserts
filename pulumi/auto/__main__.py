...
### 1 STEP: Prepare new docker image for application ###
application_image = build.Build(service_name, build.BuildArgs())

front=deploy.WebService(f"{service_name}", deploy.WebServiceArgs(
    ...
    image_name=application_image.app_image.base_image_name,
